...
const Thumbnail = ({ item, title }: { item?: any; title: string }) => {
  return (
    <ContentFrame title={title}>
      <div className='rounded-xl bg-neutral-3 overflow-hidden'>
        <div className='h-[180px] w-full relative bg-royalBlue flexCenter'>
          <Image
            src={
              item.coverPhoto?.location
                ? item.coverPhoto?.location
                : '/images/blank-banner.png'
            }
            fill
            alt='banner'
          ...
          />
          <div className='flexCenter flex-col text-neutral-1 z-10 p-3 bg-royalBlue bg-opacity-40 rounded-2xl'>
        ...
        <div className='h-[120px] relative px-12 py-[18px] flex'>
          <div className='relative flexCenter -translate-y-10 rounded-md overflow-hidden w-[115px] h-[115px] bg-royalBlue-60 shrink-0 shadow-banner'>
            <Image
              src={
                item.logo?.location
                  ? item.logo?.location
          ...
          <div className='mx-12 grow'>
            ...
            <div className='flex'>
              {item.categories?.map((cat: any, index: number) => (
                <Tag type='line' className='h-[18px] text-xs' key={index}>
                  {cat.name}
                </Tag>
              ))}
            </div>
