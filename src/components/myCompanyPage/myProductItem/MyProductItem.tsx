...
const MyProductItem = ({ companyId }: { companyId: string }) => {
  ...
  const searchParam = useSearchParams().get('product-id');
  //   START: fetch data
  ...
  const handleEditProduct = (productId: string) => {
    ...
    router.push(
      `http://localhost:3000/dashboard/companies/${companyId}?page=product&product-id=${productId}`
    );
  };
  ...
  return !editing ? (
    ...
    <div>
      <ItemTitle
        ...
        backLink={`/dashboard/companies/${companyId}?page=product`}
